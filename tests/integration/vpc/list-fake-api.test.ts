...
describe('vpc list against a fake MyAccount API', () => {
  ...
  it('renders human VPC list output with sorted names and CIDR sources', async () => {
    const server = await startTestHttpServer({
      'GET /myaccount/api/v1/vpc/list/': () => ({
        body: {
          code: 200,
    ...
    try {
      ...
      const result = await runBuiltCli(['vpc', 'list'], {
        env: {
          HOME: tempHome.path,
          [MYACCOUNT_BASE_URL_ENV_VAR]: `${server.baseUrl}/myaccount/api/v1`
        }
      ...
      expect(result.stdout.indexOf('alpha-vpc')).toBeLessThan(
        result.stdout.indexOf('zeta-vpc')
      );
    } finally {
      await server.close();
      await tempHome.cleanup();
