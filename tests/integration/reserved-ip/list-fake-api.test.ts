...
describe('reserved-ip list against a fake MyAccount API', () => {
  ...
  it('renders human reserved-IP list output with sorted addresses and attachment labels', async () => {
    const server = await startTestHttpServer({
      'GET /myaccount/api/v1/reserve_ips/': () => ({
        body: {
          code: 200,
    ...
    try {
      ...
      const result = await runBuiltCli(['reserved-ip', 'list'], {
        env: {
          HOME: tempHome.path,
          [MYACCOUNT_BASE_URL_ENV_VAR]: `${server.baseUrl}/myaccount/api/v1`
        }
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe('');
      expect(result.stdout).toContain('IP Address');
      expect(result.stdout).toContain('node-a (VM 100157)');
      expect(result.stdout.indexOf('164.52.198.54')).toBeLessThan(
        result.stdout.indexOf('216.48.184.202')
      );
    } finally {
      await server.close();
      await tempHome.cleanup();
  ...
  it('renders a clear empty-state message for reserved-ip list', async () => {
    const server = await startTestHttpServer({
      'GET /myaccount/api/v1/reserve_ips/': () => ({
        body: {
          code: 200,
    ...
    try {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe('');
      expect(result.stdout).toBe('No reserved IPs found.\n');
    } finally {
      await server.close();
      await tempHome.cleanup();
    }
