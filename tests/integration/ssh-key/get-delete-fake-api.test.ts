...
describe('ssh-key get/delete against a fake MyAccount API', () => {
  ...
  it('renders human SSH-key detail output', async () => {
    const server = await startTestHttpServer({
      'GET /myaccount/api/v1/ssh_keys/': () => ({
        body: {
          code: 200,
    ...
    try {
      ...
      const result = await runBuiltCli(['ssh-key', 'get', '1001'], {
        env: {
          HOME: tempHome.path,
          [MYACCOUNT_BASE_URL_ENV_VAR]: `${server.baseUrl}/myaccount/api/v1`
        }
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe('');
      expect(result.stdout).toContain('Label: alpha');
      expect(result.stdout).toContain('Attached Nodes: 2');
      expect(result.stdout).toContain(
        'Public Key: ssh-ed25519 AAAAC3Nza alpha'
      );
    } finally {
      await server.close();
      await tempHome.cleanup();
