...
describe('ssl list against a fake MyAccount API', () => {
  it('renders a clear human empty state', async () => {
    ...
    try {
      ...
      const result = await runBuiltCli(['ssl', 'list'], {
        env: {
          HOME: tempHome.path,
      ...
      expect(result.stdout).toBe('No SSL certificates found.\n');
    } finally {
      await server.close();
  ...
  it('renders SSL certificate IDs as a human-readable table', async () => {
    const server = await startTestHttpServer({
      'GET /myaccount/api/v1/ssl/import-certificate/': () => ({
        body: {
          code: 200,
    ...
    try {
      ...
      expect(result.stdout).toContain('api-cert');
      expect(result.stdout).toContain('api.example.com');
      expect(result.stdout).toContain('2027-01-01');
    } finally {
      await server.close();
      await tempHome.cleanup();
    }
