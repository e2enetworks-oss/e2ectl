...
describe('load-balancer backend add against a fake MyAccount API', () => {
  it('adds a server to an existing backend group', async () => {
    const receivedPutBodies: unknown[] = [];

    const server = await startTestHttpServer({
      'GET /myaccount/api/v1/appliances/load-balancers/10/': () => ({
        body: buildGetResponse()
      }),
      'PUT /myaccount/api/v1/appliances/load-balancers/10/': (body: unknown) => {
  ...
  it('lists backend groups with backend list command', async () => {
    const server = await startTestHttpServer({
      'GET /myaccount/api/v1/appliances/load-balancers/10/': () => ({
        body: buildGetResponse()
      })
