...
describe('config set-default process flow', () => {
  it('switches the saved default alias', async () => {
    ...
    try {
      ...
      const result = await runBuiltCli(
        ['config', 'set-default', '--alias', 'staging'],
        {
          env: {
            HOME: tempHome.path
          }
      ...
      expect(result.stdout).toBe('Set "staging" as the default profile.\n');
  ...
  it('fails cleanly when the alias does not exist', async () => {
    ...
    try {
      const result = await runBuiltCli(
        ['config', 'set-default', '--alias', 'missing'],
        {
          env: {
            HOME: tempHome.path
          }
