...
describe('DbaasService', () => {
  ...
  it('paginates whitelisted IPs across pages and returns the union', async () => {
    const { listWhitelistedIps, service } = createServiceFixture();
    const pageOne = Array.from({ length: 100 }, (_, index) => ({
      ip: `10.0.0.${index + 1}`
    }));
    const pageTwo = [{ ip: '10.0.1.1' }, { ip: '10.0.1.2' }];

    listWhitelistedIps.mockImplementation(
      (_dbaasId: number, pageNumber: number) =>
        Promise.resolve({
          items: pageNumber === 1 ? pageOne : pageTwo,
          total_count: 102,
          total_page_number: 2
        })
    );

    const result = await service.listWhitelistedIps('7869', { alias: 'prod' });

    expect(listWhitelistedIps).toHaveBeenNthCalledWith(1, 7869, 1, 100);
    expect(listWhitelistedIps).toHaveBeenNthCalledWith(2, 7869, 2, 100);
    expect(result.items).toHaveLength(102);
    expect(result.total_count).toBe(102);
    expect(result.items[0]?.ip).toBe('10.0.0.1');
    expect(result.items[101]?.ip).toBe('10.0.1.2');
  });
