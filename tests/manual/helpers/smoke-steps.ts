...
export async function runDnsSteps(
  context: SmokeStepContext,
  options: {
    createIpAddress: string;
    dnsRecordHost: string;
    initialRecordValue: string;
    updatedRecordValue: string;
  }
): Promise<void> {
  const dnsCreate = await runJsonCommand<DnsCreateJson>(
    [
      'dns',
      'create',
      context.smokeEnv.dnsCreateDomain,
      '--ip',
      options.createIpAddress
    ],
    context.smokeEnv
  );

  if (
    dnsCreate.action !== 'create' ||
    !Number.isInteger(dnsCreate.domain.id) ||
    dnsCreate.requested.ip_address !== options.createIpAddress
  ) {
    throw new Error('Expected dns create to return the created zone id.');
  }

  await updateManifest(context, (manifest) => {
    manifest.created_dns_domain = context.smokeEnv.dnsCreateDomain;
    manifest.created_dns_domain_deleted = false;
    manifest.created_dns_domain_id = dnsCreate.domain.id;
  });

  const dnsGet = await runJsonCommand<DnsGetJson>(
    ['dns', 'get', context.smokeEnv.dnsCreateDomain],
    context.smokeEnv
  );

  if (dnsGet.domain.domain_name.length === 0) {
    throw new Error('Expected dns get to return the created domain.');
  }

  const dnsDelete = await runJsonCommand<DnsDeleteJson>(
    ['dns', 'delete', context.smokeEnv.dnsCreateDomain, '--force'],
    context.smokeEnv
  );

  if (dnsDelete.cancelled !== false) {
    throw new Error('Expected dns delete to delete the created domain.');
  }
  ...
  const dnsRecordCreate = await runJsonCommand<DnsRecordCreateJson>(
    [
      ...
      'record',
      'create',
      context.smokeEnv.dnsDomain,
      '--type',
      'A',
      '--name',
      options.dnsRecordHost,
      '--value',
      options.initialRecordValue,
      '--ttl',
      context.smokeEnv.recordTtl
    ],
    context.smokeEnv
  );
  ...
  const dnsRecordList = await runJsonCommand<DnsRecordListJson>(
    ['dns', 'record', 'list', context.smokeEnv.dnsDomain],
    context.smokeEnv
  );

  const listedRecord = dnsRecordList.items.find(
    (item) =>
      item.name === dnsRecordCreate.record.name &&
      item.type === dnsRecordCreate.record.type &&
      item.value === options.initialRecordValue
  );

  if (listedRecord === undefined) {
    throw new Error('Expected dns record list to include the created record.');
  }
  ...
  if (dnsRecordDelete.cancelled !== false) {
    throw new Error('Expected dns record delete to remove the created record.');
  }
