...
### 412 during `load-balancer create`
...
Fix: Run `e2ectl load-balancer plans` and copy the plan name from the Plan column (for example `E2E-LB-2`). The plan name is case-sensitive and must match exactly.

### NLB_SINGLE_BACKEND_GROUP

Symptom: `NLB <id> already has a backend group. NLB supports only one backend group.`

Cause: You ran `backend group create` on an NLB that already has a backend group. NLBs support exactly one backend group.

Fix: Use `e2ectl load-balancer backend server add <lbId>` to add a server to the existing group instead of creating a new one.
